...
## Purpose
Preserve useful structure from the historical Eco-Kin Dex V1/V2 work and later image-generation chats without allowing superseded stats, attribute families, stage labels, storage/trading systems, generated image captions or roster counts to overwrite current Echohearts canon.
...
## Current repost additions
The September repost includes additional SOURCE material that belongs in this same reconciliation queue rather than becoming a second roster:
- a nature-power roster image request;
- Bug-Kin field-guide and creature-compendium image passes;
- Nerekth, the Memory Weaver;
- Skullkin, the Memory Keeper;
- Nectlume / Petal Drifters art source;
- Verdanok art source;
- the extended Guardian Beast art/name batch tracked in `07_Art/GUARDIAN_BEAST_REPOST_INTAKE_2026-09-20.md`.

These names are not promoted merely because a historical assistant described an image as canon. Each must pass the permanent identity, duplicate-DNA, originality, anatomy, element, ecology and story checks below.
...
Historical Nature, Light, Shadow, Star, Sound, Psychic, Frost, Toxic, Beast, Crystal, Aether, Ether, Cosmos, Time, Void, Atmosphere, Life and similar labels must be mapped to current elements, creature class, status language, ability tags, rarity/family metadata or visual descriptors instead of becoming extra core elements.

### Historical attribute families
Veridian, Cipher and Hollow may remain only where current Resonance Arena or historical classification work explicitly uses them. They do not replace the locked 12-element system. Primal, Celestine, Abyssal, Forge, Harmonix and Mythic Rare are not automatic core attributes; route them into class, rarity, ecology, role or presentation as appropriate.

### Nature powers
The historical request to give each Eco-Kin one nature power is preserved as a design exercise, not a universal rule. Nature/ecology abilities must be species-appropriate and map through the current element/status/ability-tag architecture. Do not force every species to gain Flora or a generic healing move.
...
Repeated generated names with incompatible concepts must be treated as collision evidence, not as multiple canon forms, unless an intentional alternate-form relationship is later approved. The Guardian Beast collision ledger is the current reference for the largest cluster of those errors.
...
Final status: LEGACY DEX V2 + IMAGE-CHAT SOURCES PRESERVED / PARTIALLY REUSABLE / NOT A SECOND DEX.